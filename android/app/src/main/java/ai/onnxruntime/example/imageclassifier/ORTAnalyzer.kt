...
internal class ORTAnalyzer(
                ...
                tensor.use {
                    ...
                    if (output != null) {
                        output.use {
                            ...
                            // in case of int8 model outputs
                            // val rawOutput = ((output?.get(0)?.value) as Array<ByteArray>)[0]
                            // val intOutput = rawOutput.map { it.toInt() and 0xFF }.toIntArray()
                            // val floatOutput = intOutput.map { it.toFloat() }.toFloatArray()
                            // result.detectedIndices = getTop3(floatOutput)
                        ...
                        Log.e("ORTAnalyzer", "Output is null")
                    }
                }
            }
            callBack(result)
    ...
    fun analyzeBitmap(bitmap: Bitmap) {
        val rawBitmap = Bitmap.createScaledBitmap(bitmap, 224, 224, false)
        ...
        val env = OrtEnvironment.getEnvironment()
        env.use { environment ->
            val tensor = OnnxTensor.createTensor(environment, imgData, shape)
